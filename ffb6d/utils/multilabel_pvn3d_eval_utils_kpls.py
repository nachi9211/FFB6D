...
# ###############################LineMOD Evaluation###############################
#todo: check cal_frame_poses_lst and set use_ctr_clus_flter=True: PROB DONE, check bs_util calls: get_cls_name, get_kps, get_ctr, project_p3d
def cal_frame_poses_lm2(
    pcld, mask, ctr_of, pred_kp_of, use_ctr, n_cls, use_ctr_clus_flter, kp_type='farthest',
    debug=False
):
    """
    Calculates pose parameters by 3D keypoints & center points voting to build
    ...
    if use_ctr_clus_flter:
        ...
        try:
            ...
            for cls_id in pred_cls_ids:
                if cls_id == 0:
                    break
                min_msk = min_dis < min_dis < config.lm_r_lst[cls_id][
                    'diameter'] * 0.4  # config.ycb_r_lst[cls_id-1] * 0.8
                update_msk = (mask > 0) & (msk_closest_ctr == cls_id) & min_msk
                new_msk[update_msk] = msk_closest_ctr[update_msk]
            mask = new_msk
        except Exception:
    ...
    for icls, cls_id in enumerate(pred_cls_ids):
        ...
        if debug:
            show_kp_img = np.zeros((480, 640, 3), np.uint8)
            kp_2ds = bs_utils.project_p3d(cls_kps[cls_id].cpu().numpy(), 1000.0, K='linemod')
            color = bs_utils.get_label_color(cls_id.item())
            show_kp_img = bs_utils.draw_p2ds(show_kp_img, kp_2ds, r=3, color=color)
            imshow("kp: cls_id=%d" % cls_id, show_kp_img)
            waitKey(0)
        ...
        # Get mesh keypoint & center point in the object coordinate system.
        # If you use your own objects, check that you load them correctly.
        mesh_kps = bs_utils_lm.get_kps(cls_lst_lm[cls_id - 1], kp_type=kp_type, ds_type="linemod")      #-1s cz lst, might have to change...
        if use_ctr:
            mesh_ctr = bs_utils_lm.get_ctr(cls_lst_lm[cls_id - 1], ds_type="linemod").reshape(1, 3)
            mesh_kps = np.concatenate((mesh_kps, mesh_ctr), axis=0)
        pred_kpc = cls_kps[cls_id].squeeze().contiguous().cpu().numpy()
        pred_RT = best_fit_transform(mesh_kps, pred_kpc)
        pred_kps_lst.append(pred_kpc)
...
def cal_frame_poses_lm1(
    ...
    pred_pose_lst = []
    #NachiN: make this cls_id change according to object? how will that work though?
    cls_id = 1
    cls_msk = mask == cls_id
    if cls_msk.sum() < 1:
        pred_pose_lst.append(np.identity(4)[:3, :])
        ...
        cls_voted_kps = pred_kp[:, cls_msk, :]
        ms = MeanShiftTorch(bandwidth=radius)
        #NachiN: use ms.fit_multi_clus instead, p&c
        ctr, ctr_labels = ms.fit(pred_ctr[cls_msk, :])
        #ctr, ctr_labels = ms.fit_multi_clus(pred_ctr[cls_msk, :])
        if ctr_labels.sum() < 1:
            ctr_labels[0] = 1
...
#todo: adapt, PROB DONE
def eval_metric_lm2(cls_ids, pred_pose_lst, pred_cls_ids, RTs, mask, label,
    gt_kps, gt_ctrs, pred_kpc_lst):  #, obj_id
    n_cls = config.n_classes
    cls_add_dis = [list() for i in range(n_cls)]
    cls_adds_dis = [list() for i in range(n_cls)]
    cls_kp_err = [list() for i in range(n_cls)]
...
def eval_metric_lm1(cls_ids, pred_pose_lst, RTs, mask, label, obj_id):
    n_cls = config.n_classes
    cls_add_dis = [list() for i in range(n_cls)]
    cls_adds_dis = [list() for i in range(n_cls)]
...
#todo: adapt: MAYBE DONE
def eval_one_frame_pose_lm2(
    item
):
    '''
    pcld, mask, ctr_of, pred_kp_of, RTs, cls_ids, use_ctr, n_cls, \
    ...
    pred_cls_ids, pred_pose_lst, pred_kpc_lst = cal_frame_poses_lm2(
        pcld, mask, ctr_of, pred_kp_of, use_ctr, n_cls, use_ctr_clus_flter,
        gt_kps, gt_ctrs, kp_type=kp_type
    )

    cls_add_dis, cls_adds_dis, cls_kp_err = eval_metric_lm2(
        cls_ids, pred_pose_lst, pred_cls_ids, RTs, mask, label, gt_kps, gt_ctrs,
        pred_kpc_lst
    )
    return (cls_add_dis, cls_adds_dis, pred_cls_ids, pred_pose_lst, cls_kp_err)

def eval_one_frame_pose_lm1(
    item
):
    pcld, mask, ctr_of, pred_kp_of, RTs, cls_ids, use_ctr, n_cls, \
        min_cnt, use_ctr_clus_flter, label, epoch, ibs, obj_id = item
    pred_pose_lst = cal_frame_poses_lm1(
        pcld, mask, ctr_of, pred_kp_of, use_ctr, n_cls, use_ctr_clus_flter,
        obj_id
    )

    cls_add_dis, cls_adds_dis = eval_metric_lm1(
        cls_ids, pred_pose_lst, RTs, mask, label, obj_id
    )
    return (cls_add_dis, cls_adds_dis)
...
class TorchEval():
    ...
    def cal_lm_add1(self, obj_id, test_occ=False):
        add_auc_lst = []
        adds_auc_lst = []
        add_s_auc_lst = []
        cls_id = obj_id
    ...
    #todo: entire function unreviewed, v imp, PROB DONE
    def eval_pose_parallel2(
        ...
        if ds == "ycb":
            ...
            data_gen = zip(
                pclds, masks, pred_ctr_ofs, pred_kp_ofs, RTs,
                cls_ids, use_ctr_lst, n_cls_lst, min_cnt_lst, use_ctr_clus_flter_lst,
                labels, epoch_lst, bs_lst, gt_kps, gt_ctrs, kp_type     #, obj_id_lst
            )
        with concurrent.futures.ThreadPoolExecutor(
            ...
            if ds == "ycb":
                ...
                eval_func = eval_one_frame_pose_lm2
    ...
    def eval_pose_parallel1(
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            if ds == "ycb":
                ...
                eval_func = eval_one_frame_pose_lm1
            for res in executor.map(eval_func, data_gen):
                if ds == 'ycb':
                    cls_add_dis_lst, cls_adds_dis_lst, pred_cls_ids, pred_poses, pred_kp_errs = res
                    self.pred_id2pose_lst.append(
